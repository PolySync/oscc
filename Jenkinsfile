  ...
  try {
    ...
    stage('Kia Soul Petrol Tests') {
      parallel 'kia soul petrol firmware unit tests': {
        ...
        sh 'cd api/tests && mkdir build_kia_soul_ev_property_tests && cd build_kia_soul_ev_property_tests && $HOME/.cargo/bin/rustup override set 1.17.0 && cmake .. -DKIA_SOUL=ON && make run-api-property-tests ; $HOME/.cargo/bin/rustup override unset'
    ...
    stage('Kia Soul EV Tests') {
      parallel 'kia soul ev unit tests': {
        ...
        sh 'cd api/tests && chmod +x initialize_vcan.sh && ./initialize_vcan.sh'
        sh 'cd api/tests && mkdir build_kia_soul_ev_property_tests && cd build_kia_soul_ev_property_tests && $HOME/.cargo/bin/rustup override set 1.17.0 && cmake .. -DKIA_SOUL_EV=ON && make run-api-property-tests ; $HOME/.cargo/bin/rustup override unset'
        sh 'sudo ip link set vcan0 down && sudo rmmod vcan'
