WHEN("^the time since the last heartbeat report publishing exceeds the interval$")
{
    ...
    g_tx_chassis_state_3.timestamp = OSCC_REPORT_CHASSIS_STATE_3_PUBLISH_INTERVAL_IN_MSEC;
...
WHEN("^the time since the last Chassis State 3 report publishing exceeds the interval$")
{
    ...
    g_tx_chassis_state_3.timestamp = 0;

    g_mock_arduino_millis_return = OSCC_REPORT_CHASSIS_STATE_3_PUBLISH_INTERVAL_IN_MSEC;
...
THEN("^a Chassis State 3 report should be published to the control CAN bus$")
{
    assert_that(
        g_mock_mcp_can_send_msg_buf_id,
        is_equal_to(OSCC_REPORT_CHASSIS_STATE_3_CAN_ID));
    ...
    assert_that(
        g_mock_mcp_can_send_msg_buf_len,
        is_equal_to(OSCC_REPORT_CHASSIS_STATE_3_CAN_DLC));

    assert_that(
        g_tx_chassis_state_3.timestamp,
        is_equal_to(g_mock_arduino_millis_return));
}
