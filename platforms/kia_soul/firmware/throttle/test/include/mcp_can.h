...
typedef void (*rust_callback)(uint32_t id, uint8_t ext, uint8_t len, uint8_t *buf);
public:
    MCP_CAN(INT8U _CS);
    ...
    void register_can_frame(INT32U id, INT8U ext, INT8U len, INT8U *buf);

private:
    rust_callback cb;
    INT32U _id;
    INT8U _len;
    INT8U _buf[CAN_MAX_CHAR_IN_MESSAGE];
};
