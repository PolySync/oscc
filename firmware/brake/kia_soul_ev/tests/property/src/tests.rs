...
fn prop_only_process_valid_messages(
    ...
    unsafe {
        ...
        TestResult::from_bool(
            g_brake_control_state.enabled == enabled &&
                g_brake_control_state.operator_override == operator_override &&
                g_brake_control_state.dtcs == dtcs && g_mock_dac_output_a == dac_a &&
...
fn check_process_enable_command() {
    QuickCheck::new().tests(1000).quickcheck(
        prop_process_enable_command as
            fn(oscc_brake_enable_s) -> TestResult,
    )
}
...
fn prop_output_accurate_spoofs(mut brake_command_msg: oscc_brake_command_s) -> TestResult {
    brake_command_msg.spoof_value_low = rand::thread_rng().gen_range(
        BRAKE_SPOOF_LOW_SIGNAL_RANGE_MIN as u16,
        BRAKE_SPOOF_LOW_SIGNAL_RANGE_MAX as u16,
    );
    brake_command_msg.spoof_value_high = rand::thread_rng().gen_range(
    ...
    unsafe {
        ...
        TestResult::from_bool(
            g_mock_dac_output_b == brake_command_msg.spoof_value_low &&
...
fn check_output_accurate_spoofs() {
    QuickCheck::new()
        ...
        .quickcheck(
            prop_output_accurate_spoofs as fn(oscc_brake_command_s) -> TestResult,
        )
}
...
fn check_output_constrained_spoofs() {
    QuickCheck::new()
        ...
        .gen(StdGen::new(rand::thread_rng(), u16::max_value() as usize))
        .quickcheck(
            prop_output_constrained_spoofs as fn(oscc_brake_command_s) -> TestResult,
        )
}
...
fn prop_send_valid_can_fields(enabled: bool, operator_override: bool, dtcs: u8) -> TestResult {
    unsafe {
        ...
        TestResult::from_bool(
            (*brake_report_msg).enabled == enabled as u8 &&
