    ...
    const uint8_t * const data )
{
    if ( data != NULL )
    {
        ...
        if ( (config->name == OSCC_CONFIG_U16_BRAKE_EV_SPOOF_LOW_SIGNAL_RANGE_MIN)
            || (config->name == OSCC_CONFIG_U16_BRAKE_EV_SPOOF_LOW_SIGNAL_RANGE_MAX)
            || (config->name == OSCC_CONFIG_U16_BRAKE_EV_SPOOF_HIGH_SIGNAL_RANGE_MIN)
            || (config->name == OSCC_CONFIG_U16_BRAKE_EV_SPOOF_HIGH_SIGNAL_RANGE_MAX)
            || (config->name == OSCC_CONFIG_U16_BRAKE_EV_PEDAL_OVERRIDE_THRESHOLD)
            || (config->name == OSCC_CONFIG_U16_BRAKE_EV_FAULT_CHECK_FREQUENCY_IN_HZ)
            || (config->name == OSCC_CONFIG_U16_BRAKE_EV_REPORT_PUBLISH_FREQ_IN_HZ) )
        {
