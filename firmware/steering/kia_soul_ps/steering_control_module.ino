...
// ms
#define PS_CTRL_RX_WARN_TIMEOUT 150//(50)
...
uint16_t TSensL_current,        // Current measured torque sensor values
         TSensH_current,
         TSpoofH,               // Current spoofing values
         TSpoofL;
...
void enableControl() 
{
	...
	TSensL_current = (readingsL / AVG_max) << 2;
	TSensH_current = (readingsH / AVG_max) << 2;
...
void disableControl() {
	...
	TSensL_current = readingsL / AVG_max;
	TSensH_current = readingsH / AVG_max;
...
void calculateTorqueSpoof( float torque ) 
{
...
static void process_ps_ctrl_steering_command( const uint8_t * const rx_frame_buffer )
{
    ...
    current_ctrl_state.commanded_steering_angle = control_data->steering_wheel_angle_command / 90 ;

    current_ctrl_state.commanded_torque = control_data->torque;
...
static void process_psvc_chassis_state1( const uint8_t * const rx_frame_buffer )
{
	...
	// convert from 40 degree range to 470 degree range in 0.1 deg increments
	current_ctrl_state.current_steering_angle *= 117;
...
void setup() 
{
    ...
    //steeringPID.SetOutputLimits(-1500, 1500); // TODO: IMPLEMENT THIS RANGE IN POLYSYNC DRIVER 
    //steeringPID.SetSampleTime(50);
...
void loop()
{
    ...
    TSensL_current = analogRead(TSENS_LOW) << 2;  //10 bit to 12 bit
    TSensH_current = analogRead(TSENS_HIGH) << 2;
    ...
    if ( current_ctrl_state.control_enabled ) 
	{
        ...
        // check to see if someone is grabbing the steering wheel
        // TODO: condition input so this isn't tripped from fast turning
        if( (int)(abs)(TSensH_current - TSensL_current) >= STEERING_WHEEL_CUTOFF_THRESHOLD ) 
		{
            DEBUG_PRINT("Steering wheel is torqed");
        }

        // if not, go ahead and command the steering wheel
        else 
		{
            calculateTorqueSpoof( current_ctrl_state.commanded_torque );
